...
class GaussianProcessFramework(override val model: GaussianProcessSearch, val nSamples: Int) : EvoAlg {
    ...
    override fun runTrial(evaluator: SolutionEvaluator, nEvals: Int): DoubleArray {
        ...
        repeat(nEvals) {
            val nextPoint = model.getParameterSearchValues()
            val finalScore = (0 until nSamples).map { evaluator.evaluate(nextPoint) }.average()
            model.addPoint(nextPoint, finalScore)
        }
